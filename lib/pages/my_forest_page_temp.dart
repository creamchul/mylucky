...
class _MyForestPageState extends State<MyForestPage> with TickerProviderStateMixin {
  ...
  int _getTotalFocusTime() {
    return _completedSessions.fold(0, (sum, session) {
      // ?§Ï†ú ÏßëÏ§ë???úÍ∞Ñ??Î∂??®ÏúÑÎ°?Í≥ÑÏÇ∞
  ...
  String _getSessionTimeText(FocusSessionModel session) {
    if (session.isStopwatchMode) {
      // ?§ÌÜ±?åÏπò Î™®Îìú: ?§Ï†ú Í≤ΩÍ≥º ?úÍ∞Ñ ?úÏãú
      final minutes = session.elapsedSeconds ~/ 60;
      final seconds = session.elapsedSeconds % 60;
      if (minutes > 0) {
        return seconds > 0 ? '${minutes}Î∂?${seconds}Ï¥?ÏßëÏ§ë' : '${minutes}Î∂?ÏßëÏ§ë';
      } else {
        return '${seconds}Ï¥?ÏßëÏ§ë';
      }
    } else {
      // ?Ä?¥Î®∏ Î™®Îìú: ?§Ï†ï???úÍ∞Ñ ?úÏãú
      return '${session.durationMinutesSet}Î∂?ÏßëÏ§ë';
    }
  }
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'ÏßëÏ§ë?òÍ∏∞Î•??µÌï¥ Ï≤?Î≤àÏß∏ ?òÎ¨¥Î•??¨Ïñ¥Î≥¥ÏÑ∏??\nÎß§Ïùº Ï°∞Í∏à??ÏßëÏ§ë?òÎ©¥ ?ÑÎ¶Ñ?§Ïö¥ ?≤Ïù¥ ÎßåÎì§?¥Ïßë?àÎã§.',
                style: TextStyle(
                  fontSize: 16,
                  color: Colors.grey.shade600,
                  height: 1.5,
              ...
              ElevatedButton.icon(
                ...
                label: const Text('ÏßëÏ§ë?òÎü¨ Í∞ÄÍ∏?),
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      '${widget.currentUser.nickname}?òÏùò ??,
                      style: TextStyle(
                        fontSize: 20,
                        fontWeight: FontWeight.bold,
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(
              title.contains('?±Ïû•') ? Icons.nature : Icons.eco,
              size: 20,
              color: color,
            ),
          ),
      ...
      builder: (context, child) {
        return GridView.builder(
          ...
          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
            ...
            crossAxisSpacing: 16,
            mainAxisSpacing: 16,
            childAspectRatio: 0.9, // ?∏Î°úÎ•???Í∏∏Í≤å ?òÏó¨ ?¨Ïú† Í≥µÍ∞Ñ ?ïÎ≥¥
          ),
