...
class _PetCarePageState extends State<PetCarePage> 
  ...
  Future<void> _checkFirebaseConnection() async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('Firebase 연결 상태: ${isConnected ? "연결됨" : "연결 안됨"}');
      }
    } catch (e) {
      setState(() {
        _isFirebaseConnected = false;
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          titleSpacing: 0, // 제목과 leading 사이 간격 줄임
          leading: IconButton(
            icon: Icon(
          ...
          actions: [
            // Firebase 연결 상태 표시 (아이콘만)
            if (!kIsWeb)
              Container(
                margin: const EdgeInsets.only(right: 4),
                padding: const EdgeInsets.all(6),
                decoration: BoxDecoration(
                  color: _isFirebaseConnected ? Colors.green.shade100 : Colors.orange.shade100,
                  shape: BoxShape.circle,
                ),
                child: Icon(
                  _isFirebaseConnected ? Icons.cloud_done : Icons.cloud_off,
                  size: 14,
                  color: _isFirebaseConnected ? Colors.green.shade700 : Colors.orange.shade700,
                ),
              ),
            // 테스트용 포인트 추가 버튼
            IconButton(
            ...
            Container(
              margin: const EdgeInsets.only(right: 12), // 마진 줄임
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), // 패딩 줄임
              decoration: BoxDecoration(
                color: Colors.amber.shade100,
                borderRadius: BorderRadius.circular(16), // 둥글기 줄임
